...
test.describe('MetaMask + Testudo Connection', () => {
	test('Testudo extension loads alongside MetaMask', async ({
		page,
		testudoExtensionId,
	}) => {
		await page.goto('/');
		...
		await page.waitForFunction(
			() =>
				(window as unknown as { __TESTUDO_LOADED__: boolean }).__TESTUDO_LOADED__ ===
				true,
			null,
			{ timeout: 15_000 },
		);
	...
	test('Testudo and MetaMask provider coexist on page', async ({
		page,
		testudoExtensionId,
	}) => {
		await page.goto('/');
