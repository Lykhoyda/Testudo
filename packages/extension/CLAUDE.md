...
| `content.ts` | Content script | Message bridge between page ↔ background, font injection, heartbeat | 4.4 KB |
...
|-------------|-----------|---------|
| `TESTUDO_ANALYZE` | page → bg | Full EIP-7702 bytecode analysis |
| `CHECK_ADDRESS` | page → bg | Address-only check (Safe Filter → Local DB → API) |
| `TESTUDO_RESOLVE_TOKEN` | page → bg | ERC20 metadata via RPC multicall |
| `TESTUDO_GET_SETTINGS` | page → bg | Read extension settings |
| `TESTUDO_RECORD_SCAN` | page → bg | Record scan to history |
| `TESTUDO_RECORD_BLOCKED` | page → bg | Record blocked transaction |
| `TESTUDO_WHITELIST_REQUEST` | page → bg | Quick whitelist (validated against analysisCache) |
...
| decoder/token-resolver.test.ts | 13 | Well-known lookup, RPC fallback, cache |
