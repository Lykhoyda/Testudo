...
5. Check nested `AGENTS.md` files in `packages/core/` and `packages/extension/` for package-specific context
...
├── packages/
│   ├── core/                     # @testudo/core — Detection engine (294 tests)
│   │   ├── AGENTS.md             # Package-specific context
│   │   ├── rolldown.config.ts
│   │   ├── src/
│   │   │   ├── index.ts          # Public exports
│   │   │   ├── analyzer.ts       # Main orchestrator (291 lines)
│   │   │   ├── detectors.ts      # 23 detection functions
...
│   │   ├── src/
│   │   │   ├── injected.tsx      # Page-context orchestrator (561 lines)
│   │   │   ├── content.ts        # Message bridge + heartbeat (265 lines)
...
| Core tests | 294 passing | 5 test files |
| Extension tests | 89 passing | Decoder + deployer-lookup (4 test files) |
...
| Auto-Forwarder (proximity: SELFBALANCE→CALL within 15) | CRITICAL | Yes |
...
| DELEGATECALL alone (no proxy pattern) | HIGH | Yes |
| DELEGATECALL + known proxy/wallet pattern | MEDIUM | No |
...
| Unlimited approval (PUSH32 max-uint) | MEDIUM | No |
